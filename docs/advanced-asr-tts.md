...
**同声传译**：始终在当前观看页进行，不新开后台标签。有字幕跟轴翻译；无字幕约 5 秒一段走 ASR。配了 TTS 时，有字幕会先截约 4 秒原声当会话音色，之后边播边补；无字幕用该段录音当临时参考。临时参考不写 IndexedDB、不复用全局上传缓存。截取失败才退回设置里的参考音。过短、含噪声或多人混说的参考会影响音色。这不是逐字对口型。
...
| 「同声传译」（无字幕） | 请到设置填转写地址 | 仍出中文字幕，没有中文配音 |
| 「同声传译」（有字幕） | 不需要 ASR | 仍出中文字幕，没有中文配音 |
