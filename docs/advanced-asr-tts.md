...
**同声传译**：使用一个临时后台标签页提前取声，当前观看页使用原播放器作为唯一时间轴。约 5 秒一段，当前原音作为该段临时音色参考（有字幕时按采集区间合并字幕，无字幕走 ASR）。临时参考不写 IndexedDB、不复用全局上传缓存。最多两段并行识别／翻译，TTS 按原顺序生成。启动缓冲两段；下一段未准备好时，画面停在已准备内容的边界。中文音频按实际时长映射到原视频的分段时间，保持音高、跟随暂停／继续、拖动和倍速；校准时只渐进调整速度，不跳过音频内容。自动换段必须同时等到画面到达分段终点和音频自然播放结束；画面先到就停住等配音，视频最后一段同样播完尾音。拖动会取消旧任务并重新缓冲。字幕在对应分段开始播放时更新。后台积压上限 12 段，达到上限暂停后台取声，消化至 6 段再继续，观看页不会被后台取声进度牵着走。停止会取消请求和播放、恢复原声，并关闭这次创建的后台标签页。

后台页需要能够打开相同的视频；广告、登录验证或站点限制可能阻止准备，失败会明确报错，不回退到音画脱节的播放方式。这是分段时间轴同步，不是逐字对口型。过短、含噪声或多人混说的参考会影响音色效果；生成速度不足或较高倍速仍可能需要缓冲。
