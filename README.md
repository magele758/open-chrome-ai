...
- **一键总结**：卡片「一键总结」/ 输入框旁「总」。优先复用完整文稿或获取一种语言的完整字幕；拿到字幕就直接总结，不下载、切分音频。字幕不可用时才下载完整音轨并用 ASR 生成全文。总结以核心观点、论据和结论为主，时间索引仅作末尾补充；长文分段阅读最多并发两项，最终总结逐步显示。不会跟随视频播放或移动播放进度；可与同声传译同时运行，停止其中一项不影响另一项
