...
同传约每 5 秒采集一块声音，但不再将音频块直接当成一句翻译。识别结果先按源顺序合并，文本模型同时判断可确认的前缀并翻译，未说完的句尾留给下一块。请求带最近三条已确认原文和译文，术语重复确认后作为后续参考。分句返回必须完整保留原文；未提交内容达到约 15 秒源音频或 1600 字符时执行有界分段，明显未完成的强制尾句只显示文字、不配音。视频自然结束会处理剩余尾句。

同传默认以 3 段已确认配音作为缓冲目标；不足一批或还在等后半句时会释放等待、继续取声，不会因凑不齐三条卡住。采音在待处理工作达到 8 项时限流，积压到 6 项时暂停画面消化；合成音频队列另设 8 段上限。一块音频可以生成多条句段。会跟随用户暂停、恢复和倍速；拖动进度后丢弃旧位置的待播内容、半句和上下文。自动暂停画面等待时，中文配音继续播放以消化积压。识别、翻译各设 15 秒请求超时，TTS 设 30 秒超时；失败会继续处理后续内容。仍有采音、理解和配音延迟，不保证口型同步。设计与首版范围见 [语义分段方案](docs/interpret-semantic-translation-plan.md)。
...
当前侧栏同传从独立下载音轨取声，准备期间原视频暂停。缓冲目标按已完成的配音段数计数，语义分段长短不同，不等于固定的 15 / 25 / 40 秒，也不能保证慢速服务永不中断。缓冲耗尽时暂停画面补充；原句画面已走完但中文配音还未结束时，画面等待完整配音。取音完成后继续监控播放，直到末尾配音处理完毕；用户暂停优先于自动恢复。

标准 PCM 音轨中的数字静音不提交 ASR。识别异常重复时保留原音频，以两个连续半段各重识别一次；其他格式重试原片段一次。仍无法确认的结果才报告失败。临时请求失败重试一次，认证或参数错误不重试。停止或跳转会取消旧识别及其重试。日志新增 `asr.retry` 与 `audio.skipped` 的 `pcm-silence` 原因。

本地回归：`node extension/tools/test_interpret_asr.mjs`、`node extension/tools/test_interpret_flow.mjs`。
